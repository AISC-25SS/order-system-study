...
| 이름  | 역할 및 기능               |
|-----|-----------------------|
| 공승준 | 프로젝트 전체 구조 세팅 및 `member` 관련 도메인, UI |
| 조은혜 | `member` 로그인, 회원가입 기능   |
| 강도원 | `item` 클래스, UI    |
| 노성빈 | `item` service, repository |
| 김종현 | `order` 클래스    |
| 허지훈 | `order` service, repository |
| 백종빈 | `discount`, `order` UI | 
